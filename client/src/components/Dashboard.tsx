...
export function Dashboard({
	...
	const wsOpts = {
		...
		onClaimFailed: (_: string, reason: string) =>
			showToast(`Claim failed: ${reason}`),
		...
		onAckFailed: (_: string, reason: string) =>
			showToast(`Ack failed: ${reason}`),
	...
	const claimedEvents = allEvents.filter(
		(e) => e.status === "claimed" && e.claimed_by === userId,
	);
	const resolvedEvents = allEvents.filter(
		(e) => e.status === "resolved" && e.claimed_by === userId,
	);
					...
					<div className="events-grid">
						{visibleEvents.map((event) => (
							<EventCard
								...
								onClaim={
									event.status === "open" ? () => claim(event.id) : undefined
								}
								onAck={
