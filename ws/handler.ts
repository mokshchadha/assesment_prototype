...
async function validateConnection(
	...
	const isValidToken =
		payload && payload.name === name && payload.region === region;
	if (!isValidToken) {
...
async function handleAcknowledge(
	ws: any,
	msg: ClientMessage & { type: "acknowledge" },
	userId: string,
	region: Region,
) {
...
export const wsHandler = {
	...
	async open(ws: any) {
		const { name, region, token } = ws.data.query as {
			name: string;
			region: Region;
			token: string;
		};
	...
	async message(ws: any, rawMessage: unknown) {
		const { name, region } = ws.data.query as {
			name: string;
			region: Region;
			token: string;
		};
		...
		if (msg.type === "claim")
			return handleClaim(ws, msg as any, moderator.id, region);
		if (msg.type === "acknowledge")
			return handleAcknowledge(ws, msg as any, moderator.id, region);
