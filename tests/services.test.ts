...
import { sql, insertEvent, getEventById, markEventClaimed } from "../db/postgres"
...
async function seedEvent(region: Region = ASIA, payload: any = { type: "test" }): Promise<string> {
...
describe("claimEvent service", () => {
  ...
  it("fails if the event does not exist", async () => {
    const result = await claimEvent("00000000-0000-0000-0000-000000000000", MOD_MOKSH, ASIA)
